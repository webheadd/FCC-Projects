...
var load = (queryUrl, isSearched) => {
  ...
  xhr.onreadystatechange = () => {
    if (xhr.readyState == 4 && xhr.status == 200) {
      ...
      if (!isSearched) console.log("no search");
      if (isSearched) console.log("searched");
...
function searchMovie(e) {
  ...
  if (text === "") {
    ...
    toUrl = `${url}movie/popular?api_key=${api_key}&language=en-US&query=${text}&page=1&include_adult=false`;
  }
